...
export default {
   ...
   computed: {
      computedBreadcrumbs() {
         if (!this.articles_breadcrumbs || !this.articles_breadcrumbs.length)
      ...
      async fuckinMiddleware() {
         if (this.$route.query.userEmail) {
            ...
            if (Request.getAccessTokenInCookies() && this.$route.query.agent) {
               ...
               const wtf = Request.get(
                  this.$store.state.BASE_URL + '/entity/articles',
                  this.$route.query
               )
            }
            if (!Request.getAccessTokenInCookies()) {
               const refreshResponse = await this.$store.dispatch(
                  'refreshTokens'
               )
