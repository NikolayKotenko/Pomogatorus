<template>
   <div class="article-template">
      <template v-if="article">
         ...
         <div
            class="article-template__content mainContentFont"
            v-html="refactored_content"
         ></div>
      </template>
...
export default {
   ...
   methods: {
      ...
      changeIndexQuestion() {
         let questions = [
            ...document.getElementsByClassName('question_wrapper'),
         ]
      ...
      initializeContent() {
         return new Promise((resolve) => {
            if (
               JSON.parse(
                  JSON.parse(JSON.parse(this.article.inserted_components))
               ).length
            ) {
               const arr_of_components = JSON.parse(
                  JSON.parse(JSON.parse(this.article.inserted_components))
               )
               const promises = []
               ...
               arr_of_components.forEach((elem) => {
                  if (elem.component.name === 'questions') {
                     promises.push(
                        this.$store.dispatch('getComponentsById', elem)
                     )
                  } else if (elem.component.name === 'image') {
                     promises.push(
                        this.$store.dispatch('imageFromServer', elem)
                     )
               ...
               Promise.all(promises).finally(() => {
                  const arr = []
                  this.$store.state.ArticleModule.components_after_request.forEach(
                     (elem) => {
                        arr.push(elem)
                     }
                  )
                  ...
                  this.$nextTick(() => {
                     arr.forEach((elem) => {
                        setTimeout(() => {
                           ...
                           const countLayout =
                              this.$store.state.ArticleModule.countLayout
                           ...
                           this.data_of_components.push(
                              this.getStructureForInstance(elem.component)
                           )
                           this.data_of_components[
                              countLayout - 1
                           ].instance.$mount() // pass nothing
                           range.insertNode(
                              this.data_of_components[elem.index - 1].instance
                                 .$el
                           )
                           this.$store.commit('M_selectedComponent', {})
                        })
      ...
      checkTypeComponent(elem) {
         ...
         if (elem.component.name === 'questions') {
            const name = Object.prototype.hasOwnProperty.call(
               ...
               : elem.component.name
            this.$store.commit(
               'M_count_of_questions',
               elem.component[`index_${name}`]
            )
         }
      },
