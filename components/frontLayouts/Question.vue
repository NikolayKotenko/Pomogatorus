<template>
   <div
      class="question_wrapper question_hover"
   ...
   >
      ...
      <div class="question_wrapper__content__question">
         <template v-if="question_data.id_type_answer == '1'">
            <v-text-field
               ...
               :disabled="
                  !!detailed_response ||
                  (check_status && status_question.type === 'sending')
               "
               @change="changeAnswer()"
            >
            </v-text-field>
                           ...
                           <v-tooltip bottom>
                              <template v-slot:activator="{ on, attrs }">
                                 <img
                                    class="help_img"
                                    :src="
                                       require(`/assets/svg/help-circle.svg`)
                                    "
                                    alt="help"
                                    v-bind="attrs"
                                    v-on="on"
         ...
         <template v-else-if="question_data.id_type_answer == '6'">
            <span
               >Укажите число в диапозоне от {{ value_type_answer[0].answer }} и
               до {{ value_type_answer[1].answer }}</span
            >
            <v-text-field
               solo
               dense
      ...
      <div
         class="py-3 file_input"
         v-if="question_data.state_attachment_response"
      >
         <template v-if="(answer || detailed_response) && !disableBtn">
            ...
            <v-btn
               ...
               :disabled="
                  (!!uploadedFiles.length && statusFile) ||
                  status_name === 'sending'
               "
         ...
         <template v-else>
            <v-tooltip bottom>
               ...
               <span
                  >Сначала необходимо выбрать вариант ответа и
                  авторизоваться!</span
               >
            </v-tooltip>
         </template>
      ...
      <div v-if="files.length" class="files_chips">
         ...
         <div class="files_chips__wrapper">
            <div
               ...
               class="files_chips__wrapper__chip"
            >
               <v-chip
                  small
                  :key="index"
               ...
               <span class="files_chips__wrapper__chip__type">{{
                  file.name.split('.')[1]
               }}</span>
            </div>
         </div>
...
export default {
   ...
   watch: {
      ...
      '$store.state.showCabinet': {
         handler(v) {
            if (!v) {
               if (
                  this.$store.state.idQuestionWhenModal ===
                  this.question_data.id
               ) {
                  if (
                     this.$store.state.currentObject &&
                     Object.keys(this.$store.state.currentObject).length
                  ) {
   ...
   computed: {
      ...
      status_question() {
         ...
         if (index !== -1)
            auth_block =
               this.$store.state.ArticleModule.components_after_request[index]
                  .index
   ...
   methods: {
      ...
      setDataEnv(dataEnv) {
         if (dataEnv) {
            ...
            this.data_env.data[dataEnv.data.data.column] = JSON.stringify(
               this.answer
            )
         } else {
            if (!this.answer && this.detailed_response) {
               if (this.value_type_answer && this.value_type_answer.length) {
                  this.data_env = {
                     ...
                     controller:
                        this.value_type_answer[0].dataEnv.data.controller,
                  ...
                  this.value_type_answer.forEach((elem) => {
                     this.data_env.data[elem.dataEnv.data.data.column] =
                        JSON.stringify(this.detailed_response)
                  })
               ...
               if (this.value_type_answer && this.value_type_answer.length) {
                  if (this.value_type_answer[0]?.dataEnv) {
                     ...
                     this.data_env.data[
                        this.value_type_answer[0].dataEnv.data.data.column
               ...
               if (this.answer?.dataEnv) {
                  ...
                  this.data_env.data[this.answer.dataEnv.data.data.column] =
                     JSON.stringify(this.answer)
               }
            }
         }
                        ...
                        const result = await Answers.update(
                           {
                              ...
                              id_user:
                                 this.$store.state.AuthModule.userData.user_data
                                    .id,
      ...
      getData() {
         ...
         if (
            Object.keys(this.$store.state.ArticleModule.selectedComponent)
               .length
         ) {
            console.log('TUTTUTTU')
            this.index_questions =
               this.$store.state.ArticleModule.count_of_questions
