<template>
   <v-card>
      ...
      <v-card-text class="detail_object_wrapper">
         <h3>Параметры:</h3>
         <div
            class="detail_object_block"
            v-for="(param, index) in paramsDetail"
         >
            ...
            >
               ...
               <template v-else>
                  <span v-if="isMobile">{{
                     `${param.value} - ${object_data[param.key]}`
                  }}</span>
      ...
      <v-card-actions class="detail_obj_footer">
         <v-btn text color="red lighten-1" @click="closeDetail">
            Закрыть
         </v-btn>
         <v-btn outlined color="green lighten-1" @click="saveData">
            Сохранить
         </v-btn>
      </v-card-actions>
   </v-card>
         ...
         for (let key in this.object_data) {
            if (this.isJson(this.object_data[key])) {
               if (this.isJson(JSON.parse(this.object_data[key]))) {
                  refactored[key] = JSON.parse(
                     JSON.parse(this.object_data[key])
                  )
               } else {
      ...
      async saveData() {
         await Request.put(
            this.$store.state.BASE_URL +
               `/entity/objects/${this.object_data.id}`,
            {
               id_region: parseInt(this.refactored_object_data.id_region)
                  ? parseInt(this.refactored_object_data.id_region)
                  : 0,
               lat: this.refactored_object_data.id_region
                  ? parseInt(this.object_data.id_region)
                  : 0,
               long: this.refactored_object_data.long
                  ? parseInt(this.object_data.long)
                  : 0,
               address: this.refactored_object_data.address,
               hide_manager_geotag:
                  this.refactored_object_data.hide_manager_geotag,
               notes: this.refactored_object_data.notes,
               id_type_object: parseInt(
                  this.refactored_object_data.id_type_object
               )
                  ? parseInt(this.refactored_object_data.id_type_object)
                  : 0,
               id_floor: parseInt(this.refactored_object_data.id_floor)
                  ? parseInt(this.refactored_object_data.id_floor)
                  : 0,
               id_wall: parseInt(this.refactored_object_data.id_wall)
                  ? parseInt(this.object_data.id_wall)
                  : 0,
               id_fuel_current: parseInt(
                  this.refactored_object_data.id_fuel_current
               )
                  ? parseInt(this.refactored_object_data.id_fuel_current)
                  : 0,
               id_fuel_reserve: parseInt(
                  this.refactored_object_data.id_fuel_reserve
               )
                  ? parseInt(this.refactored_object_data.id_fuel_reserve)
                  : 0,
               id_fuel_future: parseInt(
                  this.refactored_object_data.id_fuel_future
               )
                  ? parseInt(this.refactored_object_data.id_fuel_future)
                  : 0,
               id_boiler_room: parseInt(
                  this.refactored_object_data.id_boiler_room
               )
                  ? parseInt(this.refactored_object_data.id_boiler_room)
                  : 0,
               id_cold_water_source: parseInt(
                  this.refactored_object_data.id_cold_water_source
               )
                  ? parseInt(this.refactored_object_data.id_cold_water_source)
                  : 0,
               total_area: this.refactored_object_data.total_area
                  ? this.refactored_object_data.total_area
                  : '',
               heating_area: this.refactored_object_data.heating_area
                  ? this.refactored_object_data.heating_area
                  : '',
               basement_area: !!this.refactored_object_data.basement_area
                  ? this.refactored_object_data.basement_area
                  : '',
               heat_loss_total: this.refactored_object_data.heat_loss_total
                  ? this.refactored_object_data.heat_loss_total
                  : '',
               dedicated_electrical_power: this.refactored_object_data
                  .dedicated_electrical_power
               ...
               number_of_toilets: this.refactored_object_data.number_of_toilets
                  ...
                  : '',
               number_of_sinks: this.refactored_object_data.number_of_sinks
                  ? this.refactored_object_data.number_of_sinks
                  : '',
               number_of_showers: this.refactored_object_data.number_of_showers
                  ? this.refactored_object_data.number_of_showers
                  : '',
               number_of_baths: this.refactored_object_data.number_of_baths
                  ? this.refactored_object_data.number_of_baths
                  : '',
